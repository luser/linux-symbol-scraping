#!/usr/bin/env python
'''
Given a PackagesEnhanced file generated by scanpackages.py, fetch the most
...
def parse_packages(packages):
    ...
    if os.path.exists(cache_file) and os.stat(cache_file).st_mtime > os.stat(packages).st_mtime:
        return json.load(open(cache_file, 'rb'))
    for line in open(packages, 'rb'):
        if not line.startswith('BuildID:'):
            continue
        build_id, filename, package = line.strip().split()[1:]
...
def main():
    ...
    if options.packages:
        build_id_map = parse_packages(options.packages)
        if options.verbose:
